...
@section('title', 'Exchange Invoice')
                        ...
                        <section class="content invoice">
                            ...
                            {{--<div class="col-xs-12">
                                <h5 style="text-align: center;font-size: 16px;"
                                    class="page-header">
                                    <b>{{ $site_title }}</b><br><br>{{ $gen_phone }} |
                                    {{ $gen_email}}<br>
                            ...
                            <div class="row">
                                <div class="col-xs-12 table-responsive">
                                    <table class="table table-striped table-bordered">
                                        ...
                                        <tbody>
                                        <tr>
                                            <td>{{ date('d F Y',strtotime($exchange->created_at)) }}</td>
                                            <td>{{ $exchange->exchange->currency->name }}</td>
                                            <td>{{ $exchange->exchange->quantity}} - {{ $exchange->exchange->currency->name }}</td>
                                            <td>{{ $exchange->exchange->exchangeCurrency->name }}</td>
                                            <td>{{ $exchange->exchange_price }} - {{ $exchange->exchange->exchangeCurrency->name }}</td>
                                            <td>{{ $exchange->name }}</td>
                            ...
                            <div class="row">
                                <div class="col-md-4" style="border-right: 2px solid #ddd">
                                    <h4>Transaction ID : <b><i>{{ $exchange->transaction_id }}</i></b></h4>
                                    <hr>
                                    <h4>Payment ID : <b><i>{{ $exchange->payment_id }}</i></b></h4>
                                </div>
                                <div class="col-md-8">
                                    <img style="width: 100%;height: 300px" src="{{ asset('images') }}/{{ $exchange->image }}" alt="">
                                </div>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table">
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    {{ $exchange->exchange_price }} - {{ $exchange->exchange->exchangeCurrency->name }}
                                                </td>
                                            </tr>
                                            </tbody>
                                        </table>
                ...
                <div class="modal-footer">
                    ...
                    <div class="form-body">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input name="name" value="{{ $exchange->name }}" class="form-control input-lg" type="text" readonly>
                            </div>
                        </div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <input name="email" value="{{ $exchange->email }}" class="form-control input-lg" type="text" readonly>
                            </div>
                        </div>
                        <div class="form-group">
    ...
    <div class="modal fade" id="DelModal1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form method="post" action="{{ route('admin-exchange-update') }}" class="form-inline">
                        {!! csrf_field() !!}
                        <input type="hidden" name="id" class="abir_id" value="0">
